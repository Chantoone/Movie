...
$(document).ready(function () {
  ...
  if (token) {
    // Nếu có token: Hiện "Hồ Sơ" & "Đăng Xuất"
    const l1 = $("<li></li>");
    const a1 = $("<a></a>").text("Hồ Sơ").attr("id", "profile-btn");
    l1.append(a1);

    const l2 = $("<li></li>");
    const a2 = $("<a></a>").text("Đăng Xuất").attr("href", "#").attr("id", "logout-btn");
    l2.append(a2);
    ...
    // Sự kiện vào trang profile
    $("#profile-btn").on("click", function () {
      window.location.href = "../template/profile.html";
    });
  } else {
    // Nếu không có token: Hiện "Đăng Nhập / Đăng Ký"
    const l1 = $("<li></li>");
    const a1 = $("<a></a>").text("Đăng Nhập / Đăng Ký").attr("id", "login-btn");
    l1.append(a1);
    authMenu.append(l1);
